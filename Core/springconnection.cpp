...
SpringConnection::SpringConnection(SketchObject *o1, SketchObject *o2,
                                   double minRestLen, double maxRestLen,
                                   double k, const q_vec_type obj1Pos,
                                   const q_vec_type obj2Pos, bool showLine)
    : Connector(o1, o2, obj1Pos, obj2Pos, SPRING_ALPHA_VALUE,
                SPRING_DISPLAY_RADIUS, showLine),
...
void SpringConnection::initSpring(SketchObject *o1, SketchObject *o2,
                                  double minRestLen, double maxRestLen,
                                  double k, const q_vec_type obj1Pos,
                                  const q_vec_type obj2Pos, bool showLine)
{
  initConnector(o1, o2, obj1Pos, obj2Pos, SPRING_ALPHA_VALUE,
                SPRING_DISPLAY_RADIUS, showLine);
  minRestLength = minRestLen;
  maxRestLength = maxRestLen;
  stiffness = k;
...
bool SpringConnection::isInitialized() {
  return Connector::isInitialized() && stiffness != -1;
}
...
bool SpringConnection::addForce()
{
  ...
  if (object1 != NULL)
    object1->getModelSpacePointInWorldCoordinates(object1ConnectionPosition,
                                                  end1pos);
  else
    q_vec_copy(end1pos, object1ConnectionPosition);
  if (object2 != NULL)
    object2->getModelSpacePointInWorldCoordinates(object2ConnectionPosition,
                                                  end2pos);
  else
  ...
  if (length < Q_EPSILON) {
    return false;  // even if we don't have rest length 0, what direction do we
                   // push in?
                   // just return...
  }
  ...
  q_vec_scale(f1, -1, f2);
  if (object1 != NULL) object1->addForce(object1ConnectionPosition, f1);
  if (object2 != NULL) object2->addForce(object2ConnectionPosition, f2);
  return true;
}

SpringConnection *SpringConnection::makeSpring(
    SketchObject *o1, SketchObject *o2, const q_vec_type pos1,
    const q_vec_type pos2, bool worldRelativePos, double k, double minLen,
    double maxLen, bool showLine)
{
  ...
  SpringConnection *spring = new SpringConnection(o1, o2, minLen, maxLen, k,
                                                  newPos1, newPos2, showLine);
  return spring;
}
