...
int main()
{
  ...
  errors += testClearDisplayListBug();
  return errors;
}
...
void createObjects(WorldManager *world, SketchModel *model, SketchObject *&o1,
                   SketchObject *&o2, SketchObject *&o3, SketchObject *&o4)
{
...
// deleting an object would cause segfaults, with the problem being in
// removeObject
int testRemoveObject()
{
...
// This is currently testing for recurrence of Bug #807 in which some cases of
// deleting an object would cause segfaults.  The problem was in removeObject,
// but
...
int testClearDisplayListBug()
{
  ...
  QScopedPointer< WorldManager > world(new WorldManager(renderer));
  SketchObject *o1, *o2, *o3, *o4;
  assert(renderer->GetNumberOfPropsRendered() == 0);
  createObjects(world.data(), model.data(), o1, o2, o3, o4);
  vtkActor* actor1 = o1->getActor();
  vtkActor* actor3 = o3->getActor();
  world->clearObjects();
  assert(!renderer->HasViewProp(actor1));
  assert(!renderer->HasViewProp(actor3));
  return (renderer->HasViewProp(actor1) || renderer->HasViewProp(actor3)) ? 1 : 0;
}
