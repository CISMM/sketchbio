...
void SpringEditingMode::buttonPressed(int vrpn_ButtonNum)
{
    ...
    if (vrpn_ButtonNum == BUTTON_RIGHT(BUMPER_BUTTON_IDX)) {
        ...
        bool rAtEnd1;
        double rSpringDist = project->getHand(SketchBioHandId::RIGHT)
                                 .getNearestConnectorDistance();
        Connector* rSpring = project->getHand(SketchBioHandId::RIGHT)
                                 .getNearestConnector(&rAtEnd1);
...
void SpringEditingMode::doUpdatesForFrame()
{
    ...
    project->getHand(SketchBioHandId::LEFT)
        .setSelectionType(SketchBio::OutlineType::CONNECTORS);
